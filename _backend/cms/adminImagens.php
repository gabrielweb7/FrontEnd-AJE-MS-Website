<?php
  ...
  */
	...
	if($actionPost == "getTable") {
		...
		/* Create Sql */
		$sql = "select * from ".TABLE_IMAGENS." {$where}";
		...
		if(mysqlidb::num_rows()) {
			...
			for($i=0;$i<mysqlidb::num_rows();$i++) {
				...
				echo "<tr>";
				echo "<td width='150'><a href='{$fullPageLink}&action=editar&id=".$aw->id."'><div id='imagePreviewContainer'><img src='{$foto}' alt='".$aw->imagem_alt."' style=' width:100%;   background: #000000;' /></div></td>";
		...
		/* Inicia Upload */
		$fotoDir = upload::arquivo($foto, upload::getRandomName(), UPLOAD_DIR."imagens/topo/", array("jpg","jpeg","png"), 9000000); /* Max 9mb */
		...
		if($id) { 
			...
			/* sql */
			$sql = "UPDATE ".TABLE_IMAGENS." SET imagem_alt = '{$imagem_alt}', ";
			$sql .= "data_modificacao = '{$dataHoraNow}'{$titulo} {$fotoDirSql} ";
			$sql .= "WHERE id = {$id}";
		...
		/* PREPARANDO SQL PARA [ NOVO REGISTRO ] */
			...
			/* sql */
			$sql = "INSERT INTO ".TABLE_IMAGENS." (identificador, titulo, imagem_src, imagem_alt, data_criacao) ";
			$sql .= "VALUES ('{$identificadorGet}', '{$titulo}','{$fotoDir}','{$imagem_alt}', '{$dataHoraNow}')";
		...
		if(autenticacaoUsuario::getSessionId() == $id) { 
			...
			/* Redirecionar */
			jsTools::redirecionar("{$fullPageLink}&sucesso=Registros deletado com sucesso!");
		...
		/* SQL */
		$sql = "DELETE FROM ".TABLE_IMAGENS." WHERE id = {$id}";
...
<div id="app" class="reactive-app no-saved-theme header-bg-danger sidebar-type-push sidebar-state-open sidebar-tr-state-open sidebar-bg-default sidebar-option-default">
  ...
  <header id="header" class="header-wrap clearfix">
    ...
    <?php
		...
		/* headerTitle */
		$headerTitle = "<a href='".$fullPageLink."' style='color:black;'>Gerenciar Imagens (".ucfirst($infoPage["paginaTitulo"]).")</a>";
		/* include */
    ...
    <div class="container-fluid content-wrap">
        ...
        <?php
			...
			if($actionGet == "editar") { 
				...
				$sql = "select * from ".TABLE_IMAGENS." where id = {$id}"; 
							...
							<div class="row">
								<div class="col-xl-6">
								  <div class="form-group mb-4">
									...
									<input type="text" class="form-control" id="titulo" name="titulo" placeholder="Titulo da Imagem" <?php echo (autenticacaoUsuario::getNivelAcesso() != "programador")?"disabled":false; ?> value="<?php echo (isset($dataUser["titulo"]))?$dataUser["titulo"]:false;?>">
								  </div>
								</div>
								<div class="col-xl-6">
								  <div class="form-group mb-4">
					...
					<div class="panel bgc-white-dark" style="padding-bottom:1rem;">
						<div class="panel-body panel-body-p">    
							...
							<?php if($actionGet == "editar" and autenticacaoUsuario::getNivelAcesso() == "programador") { ?>
								<a href="javascript:void(0);" onclick="javascript:if(confirm('Deseja realmente deletar este registro ?')) { window.location.href='<?php echo $fullPageLink; ?>&action=delete&id=<?php echo $id; ?>'; }"><button type="button" class="btn btn-danger" style="float:right;clear:both;" id="deletarUsuario">Deletar</button>
							<?php } ?>
						</div>
					</div>
				</section>
  ...
  <?php
    $setMenu = "adminmenu-{$identificadorGet}";
	$setMenuType = "imagens";
