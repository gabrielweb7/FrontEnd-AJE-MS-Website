<?php
  ...
  */
	...
	if($actionPost == "getTable") {
		...
		$sql = "select * from ".TABLE_CONTATO." {$where}";
		...
		/* Create Sql */
		$sql = "select * from ".TABLE_CONTATO." {$where} order by data_envio desc limit $inicio,$registros";
		...
		if(mysqlidb::num_rows()) {
			...
			for($i=0;$i<mysqlidb::num_rows();$i++) {
				...
				echo "<tr class='hoverTr ".$aw->status."' style='cursor:pointer;' onclick=\"javascript:document.location.href='{$pageLink}?action=editar&filtro=".$filterString."&pag=".$pagina."&id=".$aw->id."'\">";
				echo "<td width='150'>".$aw->nome."</td>";
				echo "<td style='vertical-align: middle;'>".$aw->telefone."</td>";
				echo "<td style='vertical-align: middle;'>".$aw->email."</td>";
				echo "<td style='vertical-align: middle;'>".tools::convertDateTimeMysqlToBR($aw->data_envio)."</td>";
				echo "</tr>";
			}
		...
		/* SQL */
		$sql = "DELETE FROM ".TABLE_CONTATO." WHERE id = {$id}";
...
<div id="app" class="reactive-app no-saved-theme header-bg-danger sidebar-type-push sidebar-state-open sidebar-tr-state-open sidebar-bg-default sidebar-option-default">
  ...
  <header id="header" class="header-wrap clearfix">
    ...
    <?php
		...
		/* headerTitle */
		$headerTitle = "<a href='".$pageLink."' style='color:black;'>Contato</a>";
		/* include */
  ...
  <section id="main" class="main-wrap bgc-white-darkest" role="main">
    <div class="container-fluid content-wrap">
        ...
        <?php
				...
				$sql = "UPDATE ".TABLE_CONTATO." SET status = '1' where id = {$id}"; 
				
				/* Query */
				mysqlidb::query($sql);
				
				/* Criando Consulta SQL */
				$sql = "select * from ".TABLE_CONTATO." where id = {$id}"; 
