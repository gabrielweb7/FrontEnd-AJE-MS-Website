<?php
  ...
  */
	...
	if($actionPost == "getTable") {
		...
		/* Create Sql */
		$sql = "select *,SUBSTRING(texto, 1, 200) as texto from ".TABLE_NOTICIAS." {$where} limit 77";
		...
		if(mysqlidb::num_rows()) {
			...
			for($i=0;$i<mysqlidb::num_rows();$i++) {
				...
				echo "<tr>";
				echo "<td width='150'><a href='{$fullPageLink}&action=editar&id=".$aw->id."'>
					<div id='imagePreviewContainer'><img src='{$foto}' style=' width:100%;    background: #000000;' /></div>
				</td>";
		...
		/* Inicia Upload */
		$fotoDir = upload::arquivo($foto, upload::getRandomName(), UPLOAD_DIR."noticias/", array("jpg","jpeg","png"), 9000000); /* Max 9mb */
		...
		if($id) { 
			...
			/* sql */
			$sql = "UPDATE ".TABLE_NOTICIAS." SET titulo = '{$titulo}',subtitulo = '{$subtitulo}', texto = '{$texto}', ";
		...
		/* PREPARANDO SQL PARA [ NOVO REGISTRO ] */
			...
			/* sql */
			$sql = "INSERT INTO ".TABLE_NOTICIAS." (identificador, titulo, subtitulo, imagem_src, texto, data_criacao) ";
			$sql .= "VALUES ('{$identificadorGet}', '{$titulo}', '{$subtitulo}', '{$fotoDir}', '{$texto}', '{$dataHoraNow}')";
		...
		/* SQL */
		$sql = "DELETE FROM ".TABLE_NOTICIAS." WHERE id = {$id}";
...
<div id="app" class="reactive-app no-saved-theme header-bg-danger sidebar-type-push sidebar-state-open sidebar-tr-state-open sidebar-bg-default sidebar-option-default">
  ...
  <header id="header" class="header-wrap clearfix">
    ...
    <?php
		...
		/* headerTitle */
		$headerTitle = "<a href='".$fullPageLink."' style='color:black;'>Gerenciar Notícias (".ucfirst($infoPage["paginaTitulo"]).")</a>";
    ...
    <div class="container-fluid content-wrap">
        ...
        <?php
			...
			if($actionGet == "editar") { 
				...
				$sql = "select * from ".TABLE_NOTICIAS." where id = {$id}"; 
							...
							<div class="row">
								...
								<div class="col-xl-12">
								  <div class="form-group mb-4">
									...
									<textarea class="form-control summernote text-edit-simple" name="texto">
