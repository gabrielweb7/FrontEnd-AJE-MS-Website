<?php
  ...
  */
	...
	if(!isset($_SESSION["mactive"]) and $identificadorGet != "cms") { die("session:mactive não existe!"); }

	if($identificadorGet != "cms") { 
		/* Recebe variaveis da pagina via mysql  através do mactive */
		$infoPage = adminMenu::getVarsMenuPage($_SESSION["mactive"]);
	} else { 
		$infoPage["paginaTitulo"] = "CMS";
		$infoPage["icon_class"] = "fa fa-cog";	
	}

	/* Variaveis */
	$pageLink = "adminVars.php";
	$pageLinkGet = "categoria={$identificadorGet}";
	...
	if($actionPost == "getTable") {
		...
		if(mysqlidb::num_rows()) {
			...
			for($i=0;$i<mysqlidb::num_rows();$i++) {
				...
				if(!empty($aw->titulo)) { $aw->identificador = $aw->titulo; }
				
				echo "<tr class='hoverTr' style='cursor:pointer;' onclick=\"javascript:document.location.href='{$fullPageLink}&action=editar&id=".$aw->id."'\">";
		...
		if($id) { 
			...
			if(isset($identificador) and empty($identificador)) { 
				jsTools::redirecionar("{$fullPageLink}&action=editar&id={$id}&erro=É necessário escrever um Identificador!");
				die();
			}
...
<div id="app" class="reactive-app no-saved-theme header-bg-danger sidebar-type-push sidebar-state-open sidebar-tr-state-open sidebar-bg-default sidebar-option-default">
  ...
  <header id="header" class="header-wrap clearfix">
    ...
    <?php
    	...
    	/* faicon (icon) */
		$faicon = (isset($infoPage["icon_class"]))?$infoPage["icon_class"]:"fa fa-circle";

		/* headerTitle */
		$headerTitle = "<a href='".$fullPageLink."' style='color:black;'>Configurações (".ucfirst($infoPage["paginaTitulo"]).")</a>";
