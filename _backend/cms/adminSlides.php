<?php
  ...
  */
	...
	if($_SERVER["REQUEST_METHOD"] == "GET") { 
		...
		if((!$identificadorGet or empty($identificadorGet)) and !empty($actionGet)) { 
	...
	if($actionPost == "getTable") {
		...
		/* Create Sql */
		$sql = "select * from ".TABLE_SLIDES." {$where} order by ordem asc";
		...
		if(mysqlidb::num_rows()) {
			...
			for($i=0;$i<mysqlidb::num_rows();$i++) {
				...
				echo "<tr>";
				echo "<td width='150'><a href='{$fullPageLink}&action=editar&id=".$aw->id."'><img src='{$foto}' alt='".$aw->imagem_alt."' width='170' style='    background: #000000;' /></td>";
		...
		$label_titulo = isset($_POST["label_titulo"]) ? filter_var($_POST["label_titulo"],FILTER_SANITIZE_STRING) : null;
		$label_subtitulo = isset($_POST["label_subtitulo"]) ? filter_var($_POST["label_subtitulo"],FILTER_SANITIZE_STRING) : null;
		...
		/* Inicia Upload */
		$fotoDir = upload::arquivo($foto, upload::getRandomName(), UPLOAD_DIR."slides/topo/", array("jpg","jpeg","png"), 9000000); /* Max 9mb */
		...
		if($id) { 
			...
			/* sql */
			$sql = "UPDATE ".TABLE_SLIDES." SET label_titulo = '{$label_titulo}', label_subtitulo = '{$label_subtitulo}', imagem_alt = '{$imagem_alt}', ";
		...
		/* PREPARANDO SQL PARA [ NOVO REGISTRO ] */
			...
			/* sql */
			$sql = "INSERT INTO ".TABLE_SLIDES." (identificador, titulo, label_titulo, label_subtitulo, imagem_src, imagem_alt, data_criacao, ordem) ";
			$sql .= "VALUES ('{$identificadorGet}', '{$titulo}', '{$label_titulo}', '{$label_subtitulo}', '{$fotoDir}','{$imagem_alt}', '{$dataHoraNow}', '{$ordem}')";
		...
		if(mysqlidb::query($sql))
		{
			...
			if($actionPost == "novoRegistro") { 
				...
				/* Redirecionar */
				jsTools::redirecionar("{$fullPageLink}&action=editar&id={$id}&sucesso=Novo registro inserido com sucesso!".$erroMensagem);
			
			} else { 
			
				/* Redirecionar */
				jsTools::redirecionar("{$fullPageLink}&action=editar&id={$id}&sucesso=Registro atualizado com sucesso!".$erroMensagem);
		...
		/* SQL */
		$sql = "DELETE FROM ".TABLE_SLIDES." WHERE id = {$id}";
...
<div id="app" class="reactive-app no-saved-theme header-bg-danger sidebar-type-push sidebar-state-open sidebar-tr-state-open sidebar-bg-default sidebar-option-default">
  ...
  <header id="header" class="header-wrap clearfix">
    ...
    <?php
		...
		/* headerTitle */
		$headerTitle = "<a href='".$fullPageLink."' style='color:black;'>Gerenciar Slides (".ucfirst($infoPage["paginaTitulo"]).")</a>";
    ...
    <div class="container-fluid content-wrap">
        ...
        <?php
			...
			if($actionGet == "editar") { 
				...
				if(!$id) { jsTools::redirecionar($fullPageLink); }
				
				/* Criando Consulta SQL */
				$sql = "select * from ".TABLE_SLIDES." where id = {$id}"; 
				
				/* Query */
				mysqlidb::query($sql);
				
				/* Se caso resultado for negativo DIE :D */
				if(mysqlidb::num_rows() < 1) { jsTools::redirecionar($fullPageLink); }
					...
					<div class="panel bgc-white-dark">
						...
						<div class="panel-body panel-body-p">
							<div class="row">
								...
								<div class="col-xl-6">
								  <div class="form-group mb-4">
									<label for="nome"><b>Label_Titulo</b></label>
									<input type="text" class="form-control" id="label_titulo" name="label_titulo" placeholder="Titulo do Slide" value="<?php echo (isset($dataUser["label_titulo"]))?$dataUser["label_titulo"]:false;?>">
								  </div>
								</div>
								...
								<div class="col-xl-6">
								  <div class="form-group mb-4">
									  <label for="sobrenome"><b>Label_Subtitulo</b></label>
									  <input type="text" class="form-control" id="label_subtitulo" name="label_subtitulo" placeholder="Subtitulo do Slide" value="<?php echo (isset($dataUser["label_subtitulo"]))?$dataUser["label_subtitulo"]:false;?>">
								  </div>
								</div>
        ...
        <?php
          } else { jsTools::redirecionar($fullPageLink);  }
        ?>
    </div>
  </section>
  <!--Begin Sidebar-->
  ...
  <?php
	...
	/* Filtros */
	if($titlePage == "oprograma") { $titlePage = "o programa"; }
