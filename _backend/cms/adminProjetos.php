<?php
  ...
  */
	...
	if($actionPost == "getTable") {
		...
		/* Create Sql */
		$sql = "select *,SUBSTRING(texto, 1, 200) as texto from ".TABLE_PROJETOS." {$where} limit 77";
		...
		/* Inicia Upload */
		$fotoDir = upload::arquivo($foto, upload::getRandomName(), UPLOAD_DIR."projetos/", array("jpg","jpeg","png"), 9000000); /* Max 9mb */
		...
		if($id) { 
			...
			/* sql */
			$sql = "UPDATE ".TABLE_PROJETOS." SET titulo = '{$titulo}', texto = '{$texto}', ";
		...
		/* PREPARANDO SQL PARA [ NOVO REGISTRO ] */
			...
			/* sql */
			$sql = "INSERT INTO ".TABLE_PROJETOS." (identificador, titulo, imagem_src, texto, data_criacao) ";
			$sql .= "VALUES ('{$identificadorGet}', '{$titulo}', '{$fotoDir}', '{$texto}', '{$dataHoraNow}')";
		...
		/* SQL */
		$sql = "DELETE FROM ".TABLE_PROJETOS." WHERE id = {$id}";
...
<div id="app" class="reactive-app no-saved-theme header-bg-danger sidebar-type-push sidebar-state-open sidebar-tr-state-open sidebar-bg-default sidebar-option-default">
  ...
  <header id="header" class="header-wrap clearfix">
    ...
    <?php
		...
		/* headerTitle */
		$headerTitle = "<a href='".$fullPageLink."' style='color:black;'>Gerenciar Projetos (".ucfirst($infoPage["paginaTitulo"]).")</a>";
    ...
    <div class="container-fluid content-wrap">
        ...
        <?php
			...
			if($actionGet == "editar") { 
				...
				$sql = "select * from ".TABLE_PROJETOS." where id = {$id}"; 
