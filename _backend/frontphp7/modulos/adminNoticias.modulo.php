<?php
	class adminNoticias {
		...
		private static $tableName = "admin_noticias";
	...
	public static function get_pagination_links($current_page, $total_pages, $url)
		{
		    ...
		    if ($total_pages >= 1 && $current_page <= $total_pages) {
		        ...
		        $links .= "<a href=\"{$url}1\" class=\"{$active}\">1</a>";
		        ...
		        for (; $i < min($current_page + 6, $total_pages); $i++) {
		            ...
		            $links .= "<a href=\"{$url}{$i}\" class=\"{$active}\">{$i}</a>";
		        }
		        ...
		        if($total_pages > 1) { 
		        	...
		        	$links .= "<a href=\"{$url}{$total_pages}\" class=\"{$active}\">{$total_pages}</a>";
		        }
		    }
		    return $links;
		}
